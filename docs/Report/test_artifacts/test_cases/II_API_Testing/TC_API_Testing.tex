...
\title{API Test Cases \\ \large Library Management System}
\author{Test Design Team (Coders \& Non-Coders)}
\date{June 8, 2025}

\begin{document}
\maketitle
\begin{center}
    \textbf{Note:} These test cases are designed to validate the RESTful API of the Library Management System.  They are derived from the Software Requirements Specification (SRS) and are intended to be executed using a tool like Postman.  Both user-centric scenarios and technical validations are included. 
\end{center}

\section*{II. API Test Cases}
