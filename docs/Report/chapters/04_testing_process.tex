...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        block/.style={rectangle, draw, fill=blue!10, text width=10em, text centered, rounded corners, minimum height=3em, drop shadow},
        line/.style={draw, -{Stealth[length=3mm]}},
        cloud/.style={ellipse, draw, fill=gray!10, minimum height=2em, drop shadow}
    ]
        % Nodes
        \node[block] (commit) {Developer Commits Code to Git};
        \node[block, below=of commit] (ci_trigger) {CI Pipeline Triggered (e.g., on Git Push)};
        \node[block, below=of ci_trigger, text width=12em] (build) {Build Docker Images \& Start Containers (\texttt{docker-compose -f docker-compose.test.yml up})};
        ...
        \node[block, below=of build, yshift=-4cm, text width=12em] (e2e) {Run End-to-End Tests (Cypress)};
        \node[block, below=of e2e] (report) {Generate \& Archive Test Reports (Jest HTML \& Cypress Videos/Screenshots)};
        \node[cloud, below=of report, text width=8em] (review) {Team Reviews Results \& Manages Defects};
        ...
        \path [line, dashed] (review.west) -| ++(-2.5,0) |- (commit.west);
    \end{tikzpicture}
    }
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Arrows with labels
        \path [line] (cypress) -- (client) node[arrow_label] {User Interaction};
        \path [line] (client) -- (server) node[arrow_label] {API Calls (HTTP/S)};
        \path [line] (server) -- (mongo) node[arrow_label] {DB Queries};
...
\begin{lstlisting}[language=JavaScript, caption={Example of a Backend Unit Test for \texttt{bookController.js}}, label={lst:jest\_unit\_test}]
...
\begin{lstlisting}[language=JavaScript, caption={Example of a Cypress E2E Test for User Login}, label={lst:cypress\_e2e\_test}]
...
\begin{figure}[H]
    ...
    \includegraphics[width=\textwidth]{path/to/your/jest\_report\_screenshot.png}
