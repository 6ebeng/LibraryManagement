...
% Configure itemize for compact lists in tables
\setlist[itemize]{leftmargin=1em, itemsep=0pt, parsep=0pt, topsep=0pt}

\geometry{left=1cm, right=1cm, top=2cm, bottom=2cm}

\title{Non-Functional Test Cases: Security Testing \\ \large Library Management System}
\author{Test Design Team (Balanced - Coders \& Non-Coders)}
\date{June 5, 2025}

\begin{document}
\maketitle
\begin{center}
    \textbf{Note:} These test cases are designed based on SRS security requirements  and cover both user-facing (black-box) and technical (API/server-side) security aspects. Actual results and status are to be filled during test execution.
\end{center}

\section*{IV.A Non-Functional Test Cases: Security Testing}

\begin{longtable}{|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{5cm}|>{\raggedright\arraybackslash}p{5cm}|}
